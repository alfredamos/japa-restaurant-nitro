import { authDb } from "~~/utils/db/auth.db";
...
export async function loginWithoutAuthAction(loginModel: LoginModel, secret: string){
  //----> Authenticate 
  const response = await authDb.login(loginModel, secret);
